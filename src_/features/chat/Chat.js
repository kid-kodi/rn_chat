...
import { debounce, getFileNameFromUri, getFileTypeFromMimeType } from '../../common/utils/Utils';
import { useSocket } from '../../contexts/SocketProvider';
import { hasAndroidPermission } from '../../common/utils/ImagePickerUtil';
...
import { FileJobStatus, FileJobType } from '../../common/utils/Types';
...
export default function Chat({ route }) {
  ...
  const {
    ...
    loading, loadingMore, handleForwardMessage
  } = useMessage();
  ...
  const checkIfChatExist = async (user_id) => {
    try {
      ...
      setChatInfo({
        ...
        name: response?.chat?.isGroupChat ? response?.chat?.name : response?.user?.fullName || response?.user?.email,
  ...
  const handleForwardSelected = (contact_id_list, chat_id_list) => {
    handleForwardMessage(selectedMessages, contact_id_list, chat_id_list);
