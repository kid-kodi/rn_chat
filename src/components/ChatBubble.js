...
export const ChatBubble = ({text, myInf = false, time, maxWidth}) => {
  return (
    <View style={{alignSelf: 'flex-start', maxWidth: maxWidth}}>
      ...
      <Text
        style={[bubbleStyle.timeFont, myInf ? {alignSelf: 'flex-end'} : null]}>
...
export const FileBubble = ({file, maxWidth, downloadFile}) => {
  ...
  const NormalFile = ({filename, fileJobStatus, iconColor, iconName}) => {
    return (
      <View style={bubbleStyle.fileContainer}>
        <Text style={bubbleStyle.filename} numberOfLines={1}>
          {filename}
        </Text>
  ...
  const Display = ({item}) => {
    switch (item.myType) {
      case myFileType.pdf:
        return (
          <NormalFile
            ...
            fileJobStatus={item.fileJobStatus}
            fromMyself={item.fromMyself}
            iconName={item.myType}
            iconColor={'#e46767'}
          />
        );
      ...
      case myFileType.image:
        if (
          !item.fromMyself &&
          (item.fileJobStatus === FileJobStatus.failed ||
            item.fileJobStatus === FileJobStatus.unDownloaded)
        ) {
          return (
            <View>
              <Image
  ...
  const pressEvent = () => {
    if (file.fromMyself) {
      ...
      switch (file.fileJobStatus) {
        case FileJobStatus.unDownloaded:
        case FileJobStatus.failed:
          downloadFile(file);
          break;
        case FileJobStatus.progressing:
          toast.show('正在下载中', {
            type: 'normal',
            duration: 1000,
            placement: 'top',
  ...
  return (
    <View style={{alignSelf: 'flex-start', maxWidth: maxWidth}}>
      <View
        style={
          file.fromMyself ? bubbleStyle.myFileBubble : bubbleStyle.otherBubble
        }>
      ...
      <Text
        style={[
          bubbleStyle.timeFont,
          file.fromMyself ? {alignSelf: 'flex-end'} : null,
        ]}>
        {moment(file.timestamp).format('HH:mm:ss')}
      </Text>
    </View>
  );
};
