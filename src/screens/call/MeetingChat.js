...
import {FileJobStatus, FileJobType, MessageType} from '../core/utils/Types';
import DocumentPicker from 'react-native-document-picker';
import {windowWidth} from '../core/utils/Utils';
import {getFromStorage} from '../core/utils/StorageUtils';
...
export default class MeetingChat extends Component {
  ...
  async componentDidMount() {
    ...
    this.currentUser = route.params.currentUser;
  ...
  showToolBar = () => {
    ...
    Animated.timing(this.state.toolsBarFlex, {
      toValue: 0.12,
      duration: 200,
  ...
  showSendButton = () => {
    Animated.timing(this.sendButtonWidth, {
      toValue: 75,
      duration: 200,
      useNativeDriver: false,
    }).start();
  ...
  showAddButton = () => {
    Animated.timing(this.addButtonWidth, {
      toValue: 50,
      duration: 200,
      useNativeDriver: false,
    }).start();
  ...
  downloadFile = async message => {
    try {
      const filePath = `${MeetingVariable.fileService.getBundlePath()}/${
        message.filename
      }`;
          ...
          <View style={style.sendBar}>
            ...
            <Animated.View
              ...
              <TouchableOpacity
                ...
                <Text style={style.sendText}>Envoyer</Text>
              </TouchableOpacity>
            </Animated.View>
          </View>
        </View>
          ...
          <View style={style.toolContainer}>
            <TouchableOpacity
              ...
              <Ionicons
                name={'folder-outline'}
                size={40}
                color={this.state.selected ? '#aaaaaa' : 'black'}
              />
            </TouchableOpacity>
          </View>
        </Animated.View>
